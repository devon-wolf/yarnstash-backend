...
app.get('/yarns', async(req, res) => {
  try {
    const data = await client.query('SELECT * from yarns');
...
app.get('/yarns/:id', async(req, res) => {
  try {
    ...
    const data = await client.query('SELECT * from yarns where id=$1', [id]);
...
app.put('/yarns/:id', async(req, res) => {
  try {
    ...
    const data = await client.query(
      `UPDATE yarns
      SET name = $1, brand = $2, material = $3, color = $4, yarn_weight = $5, quantity = $6, partials = $7
      WHERE id=$8
      returning *;`,
      [
        req.body.name,
